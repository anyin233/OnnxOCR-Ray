    ...
    try:
        ...
        if request.use_detection:
            ...
            try:
                ...
                dt_boxes = [
                    np.array(bbox["coordinates"])
                    for bbox in detection_result["bounding_boxes"]
                ]
            except Exception as e:
                raise HTTPException(
                    status_code=500, detail=f"Detection service error: {str(e)}"
                )
            ...
            if request.use_classification:
                ...
                try:
                    ...
                    bounding_boxes_data = [
                        {"coordinates": box.tolist()} for box in valid_boxes
                    ]
                ...
                try:
                    ...
                    if angle_results:
                        classification_info = [
                            ...
                            if result.get(
                                "rotated_image"
                            )  # Only include results with rotated images
                        ]
                    ...
                    if classification_info:
                        recognition_request_data["classification_results"] = (
                            classification_info
                        )
                ...
                for i, rec_result in enumerate(rec_results):
                    ...
                    ocr_results.append(
                        OCRResult(
                            ...
                            bounding_box=rec_result.get(
                                "bounding_box",
                                valid_boxes[i].tolist() if i < len(valid_boxes) else [],
                            ),
