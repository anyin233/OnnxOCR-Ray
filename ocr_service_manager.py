...
async def start_service(request: StartServiceRequest):
    ...
    def run_service_process():
        ...
        start_ocr_component(
            service=request.service_type, port=modelid_to_port[model_id], host="0.0.0.0"
        )
...
async def stop_service(request: StopServiceRequest):
    ...
    if request.model_id in modelid_to_port:
        ...
        modelid_to_service_type.pop(
            request.model_id, None
        )  # Remove service type mapping
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                message = (
                    f"Failed to stop service on port {port}. Response: {response.text}"
                )
        ...
        return StopServiceResponse(
            status_code=404, model_id=request.model_id, message="Service not found."
        )
...
async def inference(request: InferenceRequest):
    ...
    if request.model_id in modelid_to_port:
        ...
        try:
            ...
            return InferenceResponse(
                status_code=500, model_id=request.model_id, message=str(e)
            )
    else:
        return InferenceResponse(
            status_code=404, model_id=request.model_id, message="Service not found."
...
async def list_services():
    ...
    for model_id, port in modelid_to_port.items():
        ...
        service_info = ServiceInfo(
            model_id=model_id, service_type=service_type, port=port
        )
