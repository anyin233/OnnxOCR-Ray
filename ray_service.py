...
from microservices_ray.classification_service import (
    ClassificationService,
    ClassificationRequest,
)
from microservices_ray.recognition_service import (
    RecognitionService,
    RecognitionRequest,
    ClassificationInfo,
    RecognitionResponse,
...
@ray.serve.deployment(
    name="detection_service", ray_actor_options={"num_cpus": 0, "num_gpus": 0.007}
)
...
@ray.serve.deployment(
    name="classification_service", ray_actor_options={"num_cpus": 0, "num_gpus": 0.005}
)
...
@ray.serve.deployment(
    name="recognition_service", ray_actor_options={"num_cpus": 0, "num_gpus": 0.015}
)
...
@ray.serve.deployment(name="main_service")
...
class OCRService:
    ...
    async def ocr(self, request: OCRRequest) -> RecognitionResponse:
        ...
        classification_request = ClassificationRequest(
            image=request.image, bounding_boxes=detection_response.bounding_boxes
        )
        classification_response = (
            await self.classification_service.classify_text_angle.remote(
        ...
        recognition_response = await self.recognition_service.recognize_text.remote(
            recognition_request
        )
