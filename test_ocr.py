...
def test_new_workflow(image_path):
    ...
    try:
        response = requests.post(
            "http://localhost:5006/inference", json={"image": image_base64}, timeout=30
        )
        if response.status_code == 200:
            detection_result = response.json()
            print(f"✓ Detection successful")
            print(
                f"  Processing time: {detection_result['processing_time']:.3f} seconds"
            )
    ...
    try:
        response = requests.post(
            ...
            json={"image": image_base64, "bounding_boxes": bounding_boxes},
            timeout=30,
        )
        if response.status_code == 200:
            ...
            print(
                f"  Processing time: {classification_result['processing_time']:.3f} seconds"
            )
            print(f"  Classification results:")
            for i, result in enumerate(classification_result["results"]):
                print(
                    f"    Region {i + 1}: angle={result['angle']}°, confidence={result['confidence']:.4f}"
                )
    ...
    print("\n3. Text recognition...")
    try:
        recognition_request = {"image": image_base64, "bounding_boxes": bounding_boxes}
        ...
        response = requests.post(
            "http://localhost:5007/inference", json=recognition_request, timeout=30
        )
        if response.status_code == 200:
            recognition_result = response.json()
            print(f"✓ Recognition successful")
            print(
                f"  Processing time: {recognition_result['processing_time']:.3f} seconds"
            )
            print(f"  Recognition results:")
            for i, result in enumerate(recognition_result["results"]):
                print(
                    f"    Region {i + 1}: '{result['text']}' (confidence: {result['confidence']:.4f})"
                )
...
def test_legacy_workflow(image_path):
    ...
    try:
        response = requests.post(
            "http://localhost:5006/detect", json={"image": image_base64}, timeout=30
        )
        if response.status_code == 200:
            detection_result = response.json()
            bounding_boxes = detection_result["bounding_boxes"]
        ...
        if cropped_images:
            ...
            if response.status_code == 200:
                ...
                for i, res in enumerate(result["results"]):
                    print(
                        f"  Text {i + 1}: '{res['text']}' (confidence: {res['confidence']:.4f})"
                    )
        ...
        if response.status_code == 200:
            ...
            for i, item in enumerate(result["results"]):
                ...
                if item.get("angle") is not None:
                    print(
                        f"    Angle: {item['angle']}° (confidence: {item['angle_confidence']:.4f})"
                    )
